...
switch action
    case 'init'
        ...
        BpodSystem.GUIHandles.OptoTypeAxes = nexttile(layout, 325, [5 11]);
        ...
        BpodSystem.GUIHandles.ShortLickAxes = nexttile(layout, 415, [7 3]);
        BpodSystem.GUIHandles.LongLickAxes = nexttile(layout, 418, [7 3]);
        BpodSystem.GUIHandles.ReactionTimeAxes = nexttile(layout, 421, [7 2]);
...
global BpodSystem
handles = guiAxes({'TrialTypeAxes', 'BlockTypeAxes', 'ProbeTypeAxes', 'ISIAxes', 'OptoTypeAxes', 'OutcomeAxes', 'BlockOutcomeAxes', 'OutcomeLegendAxes', 'ShortLickAxes', 'LongLickAxes', 'ReactionTimeAxes', 'StateAxes', 'EventAxes'});
...
function handles = guiAxes(names)
global BpodSystem
handles = gobjects(0);
if ~isfield(BpodSystem, 'GUIHandles')
    return
end
for i = 1:numel(names)
    name = names{i};
    if isfield(BpodSystem.GUIHandles, name)
        handles(end + 1) = BpodSystem.GUIHandles.(name);
    end
end
end
...
rowCount = numel(display.Labels);
ylim(ax, [0.5 rowCount + 0.5]);
xticks(ax, trialTicks(firstTrial, lastTrial));
yticks(ax, 1:rowCount);
